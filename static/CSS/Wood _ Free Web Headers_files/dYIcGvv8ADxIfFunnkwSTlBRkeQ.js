;if(CloudflareApps.matchPage(CloudflareApps.installs['8JRP4qlcKLgm'].URLPatterns)){(function(){"use strict"
if(!window.addEventListener)return
var options=CloudflareApps.installs['8JRP4qlcKLgm'].options
var element
...
return max}
function inIframe(){try{if("8JRP4qlcKLgm"==='preview'){return window.top!==window.parent.parent}
...
let isConsent=!((!inIframe())&&("8JRP4qlcKLgm"==='preview'||((localStorageValid===false)||(localStorageValid===undefined))));if(!((options.callbackFunction=="")||(options.callbackFunction==null))){window.onload=function(){let fn=window[options.callbackFunction]
...
elementButton.onclick=function(){if("8JRP4qlcKLgm"!='preview'){let fn=window[options.callbackFunction]
...
window.CloudflareApps.installs['8JRP4qlcKLgm'].scope={setOptions:function setOptions(nextOptions){options=nextOptions
